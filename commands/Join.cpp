...
void Server::JOIN(void){
    ...
    for (size_t i=0; i<chans.size(); i++){
        ...
        //since i will work with map i don't need the channel checker anymore
        //i will access the channel directly with its name

        if (index <= 0){ // channel doesn't exist in _channel vector
            Channel Chan;
            Client *membr = &this->_client[this->_currentClient];
            Chan.SetName(chans[i]);
            Chan.GetMembers().push_back(*membr); //convert it to map to add client and its nickname
            Chan.GetOps().push_back(*membr); // same as members
            this->_channel.push_back(Chan); // limits of channel in server and convert it to map
            // and add chan and its name as map
            // std::cout<<Chan.GetName()<<std::endl;
            sendErr(RPL_JOIN, "");
            return ;
        }
        else{ //channel exist // check for active bans and if it invite only 
            if (this->_channel[index - 1].GetBoolPswd()){ //when channel ha password so i will check it by its name (chans[i]) and check the password using modes flags
